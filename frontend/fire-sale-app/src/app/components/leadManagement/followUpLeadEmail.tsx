// components/FollowUpLeadEmail.tsx
...
interface FollowUpLeadEmailProps {
  lead: any;
  onClose: () => void;
  previousEmail: {
...
const FollowUpLeadEmail: React.FC<FollowUpLeadEmailProps> = ({ 
  lead, 
  onClose,
  previousEmail 
}) => {
  ...
  const [formData, setFormData] = useState({
    ...
    to: lead.email,
    subject: `Re: ${previousEmail.email_subject}`,
    message: `\n\n---------- Previous Message ----------\n${previousEmail.email_body}`
  });
  ...
  const handleGenerateTemplate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        "audience_interests": ['catering'],
        "previous_email": previousEmail.email_body
      };
          ...
          {lead.derived_insights?.primary_interests?.length > 0 && (
            <div className="mt-3">
              ...
              <div className="flex flex-wrap gap-2">
                {lead.derived_insights.primary_interests.map((interest:any, index:any) => (
                  <span 
                    key={index} 
...
export default FollowUpLeadEmail;